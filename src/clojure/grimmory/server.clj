...
(defn write-log! [msg]
  (let [entry (str "[" (java.time.Instant/now) "] " msg)
    ...
    (println "[Grimmory Clojure Server]" msg)))
...
(defn app [req]
  (let [uri (:uri req)
    ...
    (cond
      ...
      :else (json-response 404 {:status "not-found" :message "Endpoint undefined in Grimmory Clojure API API"}))))
...
(defn -main [& args]
  (init-filesystem!)
  (write-log! "Grimmory Clojure-Ring Web Daemon booting on address port 3000...")
